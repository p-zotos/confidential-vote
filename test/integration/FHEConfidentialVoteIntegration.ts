...
describe("FHEConfidentialVote Integration Tests", function () {
  ...
  describe("Stage: Registration", function () {
    it("Reverts if a registered voter tries to register again", async function () {
      ...
      await expect(contract.connect(signers.voter1).register({ value: REGISTRATION_FEE })).to.be.revertedWith(
        "Already registered",
      );
    });
    it("Reverts if a voter sends insufficient Ether", async function () {
      await expect(contract.connect(signers.voter1).register({ value: INSUFFICIENT_FEE })).to.be.revertedWith(
        "Insufficient Ether sent",
      );
    });
    it("Reverts if contract owner tries to register", async function () {
      await expect(contract.connect(signers.owner).register({ value: REGISTRATION_FEE })).to.be.revertedWith(
        "Contract owner cannot perform this action",
      );
    });
  ...
  describe("Stage: Done & Winner", function () {
    it("Reverts if a non-owner tries to compute the winner", async function () {
      await expect(contract.connect(signers.voter1).revealEncryptedWinner()).to.be.revertedWith("Not contract owner");
    });
    ...
    it("Reverts if total proposals is zero when computing winner", async function () {
      ...
      await expect(contractWithZeroProposals.connect(signers.owner).revealEncryptedWinner()).to.be.revertedWith(
        "No proposals",
      );
    });
  ...
  describe("Advance Stage", function () {
    it("Owner advances Done stage", async function () {
      ...
      await expect(contract.connect(signers.owner).advanceStage()).to.be.revertedWith("Already in final stage");
    });
